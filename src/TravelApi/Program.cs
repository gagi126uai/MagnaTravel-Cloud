...
using (var scope = app.Services.CreateScope())
{
    ...
    if (dbContext.Database.IsRelational())
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            """
            CREATE TABLE IF NOT EXISTS "TravelFiles" (
                "Id" integer GENERATED BY DEFAULT AS IDENTITY,
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            "ALTER TABLE \"AspNetUsers\" ADD COLUMN IF NOT EXISTS \"AgencyId\" integer;");
            
        await dbContext.Database.ExecuteSqlRawAsync(
            """
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'FK_AspNetUsers_Agencies_AgencyId') THEN
                    ALTER TABLE "AspNetUsers" ADD CONSTRAINT "FK_AspNetUsers_Agencies_AgencyId" FOREIGN KEY ("AgencyId") REFERENCES "Agencies" ("Id") ON DELETE RESTRICT;
                END IF;
            END
            $$;
            """);
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            "ALTER TABLE \"Reservations\" ADD COLUMN IF NOT EXISTS \"TravelFileId\" integer;");

        await dbContext.Database.ExecuteSqlRawAsync(
             """
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'FK_Reservations_TravelFiles_TravelFileId') THEN
                    ALTER TABLE "Reservations" ADD CONSTRAINT "FK_Reservations_TravelFiles_TravelFileId" FOREIGN KEY ("TravelFileId") REFERENCES "TravelFiles" ("Id") ON DELETE RESTRICT;
                END IF;
            END
            $$;
            """);
        ...
        // Phase 5: Retail Pivot (Suppliers, Customer Financials, File Payer)
        await dbContext.Database.ExecuteSqlRawAsync(@"
            CREATE TABLE IF NOT EXISTS ""Suppliers"" (
                ""Id"" integer GENERATED BY DEFAULT AS IDENTITY,
        ...
        await dbContext.Database.ExecuteSqlRawAsync(@"
            DO $$ 
            BEGIN 
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='Customers' AND column_name='CreditLimit') THEN
                    ALTER TABLE ""Customers"" ADD COLUMN ""CreditLimit"" numeric(12,2) NOT NULL DEFAULT 0;
                END IF;
        ...
        await dbContext.Database.ExecuteSqlRawAsync(@"
            DO $$ 
            BEGIN 
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='TravelFiles' AND column_name='PayerId') THEN
                    ALTER TABLE ""TravelFiles"" ADD COLUMN ""PayerId"" integer NULL;
                    ALTER TABLE ""TravelFiles"" ADD CONSTRAINT ""FK_TravelFiles_Customers_PayerId"" FOREIGN KEY (""PayerId"") REFERENCES ""Customers"" (""Id"") ON DELETE RESTRICT;
                END IF;
            END $$;
        ");
    }
