...
public class HotelBookingsController : ControllerBase
{
    ...
    public async Task<IActionResult> Update(string reservaId, string id, [FromBody] UpdateHotelRequest req, CancellationToken ct)
    {
        ...
        catch (InvalidOperationException ex)
        {
            ...
            // P1 "circuito proveedor" (2026-07-21): si el rechazo es el candado de plata pagada al
            // operador sin factura (mismo Code que "anular servicio"), sumamos `code` al body SIN
            // tocar `message` — el frontend lo usara en otra tanda para ofrecer el boton "Emitir
    ...
    public async Task<IActionResult> UpdateStatus(string publicIdOrLegacyId, [FromBody] ServiceStatusUpdateRequest req, CancellationToken ct)
    {
        ...
        {
            // P1 "circuito proveedor" (2026-07-21): mismo envelope aditivo que Update (arriba).
            if (ex is ServiceCancellationRejectedException rejected)
                return Conflict(new { code = rejected.Code, message = ex.Message });
            return Conflict(new { message = ex.Message });
        }
