...
import { Loader2, XCircle, Download, FileText, ArrowUpRight, History, Receipt, AlertCircle, RotateCw, ExternalLink, ChevronDown, ChevronUp, Clock, Wallet } from "lucide-react";
...
export function HistoryTab({ payments, invoices, onDownloadPdf, onAnnulInvoice, onRetryInvoice }) {
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500 mt-6">

            {/* --- SECCIÓN COBRANZAS --- */}
            <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                    <h3 className="text-[11px] uppercase font-black text-slate-400 tracking-[0.2em] flex items-center gap-2">
                        <Wallet className="w-3.5 h-3.5" />
                        Cobranzas Recientes
                    </h3>
                    <span className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">HISTORIAL</span>
                </div>
                                                ...
                                                {i.resultado === 'A' && (
                                                    <>
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-2 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-900/20 text-slate-300 hover:text-rose-600 transition-all"
                                                            title="Anular"
                                                        >
                    ...
                    {invoices.slice(0, 10).map(i => (
                        <div key={i.id} className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                            ...
                            <div className="flex items-center gap-2 pt-4 border-t border-slate-50 dark:border-slate-800/50">
                                {i.resultado === 'A' ? (
                                    <button onClick={() => onDownloadPdf(i)} className="flex-1 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-lg text-xs font-bold shadow-lg shadow-slate-200 dark:shadow-none">DESCARGAR PDF</button>
                                ) : (
