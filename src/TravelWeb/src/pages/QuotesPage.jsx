...
export default function QuotesPage() {
    ...
    return (
        <div className="space-y-6 pb-12">
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="overflow-x-auto"><table className="min-w-full border-collapse text-left"><thead><tr className="border-b border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-800/30"><th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Cotizacion</th><th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Cliente / titulo</th><th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Destino / pax</th><th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Estado</th><th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Monto total</th><th className="px-4 py-3 pr-6 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Acciones</th></tr></thead><tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">{filtered.length === 0 ? <tr><td colSpan="6" className="px-4 py-12 text-center"><div className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-600"><FileText className="mb-3 h-10 w-10 opacity-20" /><p className="text-sm font-medium">No se encontraron cotizaciones</p></div></td></tr> : filtered.map((quote) => <tr key={getPublicId(quote)} onClick={() => loadDetail(getPublicId(quote))} className="group cursor-pointer transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/20"><td className="whitespace-nowrap px-4 py-3 align-middle"><span className="rounded-md bg-indigo-50 px-2 py-1 text-[11px] font-black text-indigo-500 dark:bg-indigo-900/20">{quote.quoteNumber}</span></td><td className="px-4 py-3 align-middle"><div className="truncate text-sm font-semibold text-slate-900 dark:text-white">{quote.title}</div><div className="truncate text-xs text-slate-400">{quote.customer?.fullName || quote.customerName || "Sin cliente asociado"}</div>{quote.leadName && <div className="truncate text-[11px] text-indigo-500">Posible cliente: {quote.leadName}</div>}<div className="mt-1 flex items-center gap-1 text-xs text-slate-400"><Calendar className="h-3 w-3" />{quote.createdAt ? new Date(quote.createdAt).toLocaleDateString() : "-"}</div></td><td className="px-4 py-3 align-middle"><div className="text-sm text-slate-600 dark:text-slate-300">{quote.destination || <span className="italic text-slate-400">Sin destino</span>}</div><div className="flex items-center gap-1 text-xs text-slate-400"><Users className="h-3 w-3" />{(quote.adults || 0) + (quote.children || 0)} pax</div></td><td className="whitespace-nowrap px-4 py-3 align-middle"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${STATUS_COLORS[quote.status] || STATUS_COLORS.Borrador}`}>{quote.status}</span></td><td className="whitespace-nowrap px-4 py-3 text-right align-middle"><div className="text-sm font-black text-indigo-600 dark:text-indigo-400">{fmt(quote.totalSale)}</div><div className="text-[10px] text-slate-400">Neto: {fmt(quote.totalCost)}</div></td><td className="whitespace-nowrap px-4 py-3 pr-4 text-right align-middle"><div className="flex justify-end gap-1"><button onClick={(event) => { event.stopPropagation(); setCreateDefaults(null); setContextLead(null); setEditingQuote(quote); setShowModal(true); }} className="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><Edit className="h-4 w-4" /></button><button onClick={(event) => { event.stopPropagation(); handleDelete(getPublicId(quote)); }} className="rounded p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-500 dark:hover:bg-rose-900/20"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}</tbody></table></div></div>
