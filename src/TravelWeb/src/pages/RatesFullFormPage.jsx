/**
 * "Carga completa" del Tarifario — el formulario largo de siempre (vigencias, variaciones
 * de habitación, aerolínea/IATA/clase/equipaje, régimen, % de menores).
 *
 * Spec firmada 2026-08-06 (§2.2/§2.3/D3): dejó de ser la puerta de entrada al Tarifario.
 * Ahora es un camino SECUNDARIO al que solo se llega desde el botón "Carga completa" de la
 * ficha en línea o de la fichita de alta rápida (RatesPage.jsx) — nunca al revés. El resto
 * del archivo es EXACTAMENTE el formulario viejo (mismo código, sin tocar su lógica) para
 * no arriesgar ninguna regresión en vigencias/variaciones de habitación, que acá siguen
 * vivas tal cual estaban.
 */
...
// Mapa valor→etiqueta ("Aereo" → "Aéreo"): la pantalla NUNCA muestra el valor crudo
// que guarda el backend (sin tilde, en Capitalización de código) — eso es jerga de
// programador, no un texto que lea un vendedor (fix 2026-08-07, hallazgo de review).
const SERVICE_TYPE_LABELS = Object.fromEntries(serviceTypes.map((tipo) => [tipo.value, tipo.label]));
...
export default function RatesFullFormPage() {
    ...
    // Prefill al llegar desde el botón "Carga completa" del Tarifario nuevo (spec 2026-08-06,
    // §2.2/§2.3): la ficha en línea o la fichita rápida mandan lo que el vendedor ya tipeó por
    // location.state, para que no tenga que escribirlo de nuevo acá.
    // Deps [] a propósito: solo nos interesa el primer render con ese state de navegación: si
    // dependiera de `location` entero, cualquier re-render con la misma navegación volvería a
    // abrir el modal de la nada.
    useEffect(() => {
        const prefill = location.state?.prefillFromRates;
        if (!prefill) return;
        const esHotel = prefill.serviceType === "Hotel";
        setForm({
            ...emptyForm,
            serviceType: prefill.serviceType || emptyForm.serviceType,
            productName: esHotel ? "" : (prefill.name || ""),
            hotelName: esHotel ? (prefill.name || "") : "",
            city: prefill.city || "",
            supplierId: prefill.supplierId || "",
            netCost: prefill.price || 0,
            salePrice: prefill.price || 0,
            currency: prefill.currency || "USD",
        });
        setShowModal(true);
        // Limpiamos el state de navegación: si el usuario refresca (F5) con Enter en el modal
        // ya abierto, no queremos que el prefill se vuelva a aplicar y le pise lo que corrigió.
    ...
    const hotelGroups = rateGroups.map(group => ({
        ...
        items: (group.items || []).map(item => ({
            ...
            roomCategory: item.serviceType === "Hotel" ? item.roomCategory : getServiceTypeLabel(item.serviceType),
        ...
        badgeLabel: `${getServiceTypeLabel(group.serviceType)} • ${getGroupMeta(group)}`,
                                            ...
                                            <tr key={getPublicId(rate)} className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 ${isExpired ? 'border-l-4 border-l-red-500 bg-red-50/10' : ''}`}>
                                                ...
                                                <td className="px-4 py-3">
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${rate.serviceType === "Aereo" ? "bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400" :
                                                        ...
                                                        {getServiceTypeLabel(rate.serviceType)}
                                                    </span>
                                                </td>
