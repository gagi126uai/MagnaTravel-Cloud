...
export default function ChangePasswordModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (formData.newPassword.length < 8) {
      showError("La contrasena nueva debe tener al menos 8 caracteres.");
      return;
    }
    ...
    try {
      ...
      showSuccess("Contrasena actualizada. Debes iniciar sesion nuevamente.");
      setFormData({ oldPassword: "", newPassword: "", confirmPassword: "" });
      onClose();
      window.dispatchEvent(new CustomEvent("auth:logout", { detail: { callServer: false } }));
    } catch (error) {
      console.error("Error changing password:", error);
      showError(error.message || "No se pudo cambiar la contrasena.");
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-foreground">Cambiar Contrasena</h2>
          ...
          <form id="change-password-form" onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">
                Contrasena Actual <span className="text-destructive">*</span>
              </label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">
                Confirmar Nueva Contrasena <span className="text-destructive">*</span>
              </label>
              <div className="relative">
                <input
