...
public class BookingServiceRescheduleTests
{
    ...
    /// <summary>
    /// ADR-053 (2026-08-13): el candado invisible DatesManuallySet quedo MUERTO (ya no hay logica que lo
    /// lea) — el escritor unico SIEMPRE recalcula, sin excepcion. Este test reemplaza al viejo
    /// "Reschedule_WithPriorManualDatesFlag_StillRecalculatesHeader" (2026-07-23): antes probaba que
    /// reprogramar bajaba la marca para poder recalcular pese a ella; ahora no hay marca que bajar, el
    /// recalculo sucede siempre.
    /// </summary>
    [Fact]
    public async Task Reschedule_AlwaysRecalculatesHeader_RegardlessOfLegacyDeadFlag()
    {
        await using var context = CreateContext();
        var reserva = await SeedMultiTypeAsync(context);
        var service = CreateService(context, CreateMapper());
